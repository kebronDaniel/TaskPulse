...
class TaskCacheIntegrationTest {
  ...
  void getTask_whenCalledTwice_usesCacheOnSecondCall() {
    ...
    Mockito.when(projectRepository.findByIdAndWorkspaceId(mockProjectUUID, mockWorkspaceUUID))
  ...
  void getTask_whenCacheEvicted_usesDbCallToGetTask() {
    ...
    TaskResponse response =
        new TaskResponse(
            mockTaskUUID, "title", "desc", TaskStatus.TODO, TaskPriority.MEDIUM, Instant.now(), 1L);
    ...
    Mockito.when(
        ...
        .thenReturn(Optional.of(task));
    Mockito.when(
            taskRepository.findByIdAndProjectIdAndDeletedAtIsNull(mockTaskUUID, mockProjectUUID))
        .thenReturn(Optional.of(task));
