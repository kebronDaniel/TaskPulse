...
public class TaskControllerTest {
  ...
  @MockitoBean private TaskFlowUserDetailsService taskFlowUserDetailsService;
  ...
  void createTask_whenRequestIsValid_returns201Created() throws Exception {
    ...
    CreateTaskRequest request =
        new CreateTaskRequest("mock title", "mock description", TaskPriority.MEDIUM, dueDate);
    ...
    Mockito.when(taskService.createTask(mockWorkspaceUUID, mockProjectUUID, request))
        .thenReturn(response);

    mockMvc
        .perform(
            post(baseUrl, mockWorkspaceUUID, mockProjectUUID)
  ...
  void createTask_whenProjectDoesNotExist_returns404NotFound() throws Exception {
    ...
    String path =
        "/api/v1/workspaces/" + mockWorkspaceUUID + "/projects/" + mockProjectUUID + "/tasks";

    ProjectNotFoundException exception = new ProjectNotFoundException(mockProjectUUID);
    Mockito.when(taskService.createTask(mockWorkspaceUUID, mockProjectUUID, request))
        .thenThrow(new ProjectNotFoundException(mockProjectUUID));
    mockMvc
        .perform(
            post(baseUrl, mockWorkspaceUUID, mockProjectUUID)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void getTask_whenRequestIsValid_returns200Ok() throws Exception {
    ...
    TaskResponse response =
        new TaskResponse(
            ...
            Instant.now(),
            0L);
    Mockito.when(taskService.getTask(mockWorkspaceUUID, mockProjectUUID, mockTaskUUID))
        .thenReturn(response);

    mockMvc
        .perform(get(baseUrl + "/{taskId}", mockWorkspaceUUID, mockProjectUUID, mockTaskUUID))
  ...
  void updateTask_whenRequestIsValid_returns200Ok() throws Exception {
    ...
    UpdateTaskRequest request =
        new UpdateTaskRequest("updated title", null, TaskPriority.LOW, null, 0L);
    TaskResponse response =
        new TaskResponse(
            mockTaskUUID,
            request.title(),
    ...
    mockMvc
        .perform(
            patch(baseUrl + "/{taskId}", mockWorkspaceUUID, mockProjectUUID, mockTaskUUID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))
                .with(csrf()))
        .andExpect(status().isOk())
    ...
    verify(taskService, Mockito.times(1))
        .updateTask(mockWorkspaceUUID, mockProjectUUID, mockTaskUUID, request);
  }
  ...
  void deleteTask_whenRequestIsValid_returns200Ok() throws Exception {
    ...
    Mockito.doNothing()
        .when(taskService)
        .deleteTask(mockWorkspaceUUID, mockProjectUUID, mockTaskUUID);
    mockMvc
        .perform(
            delete(baseUrl + "/{taskId}", mockWorkspaceUUID, mockProjectUUID, mockTaskUUID)
                .with(csrf()))
  ...
  void getTasks_requestIsValid_returns200Ok() throws Exception {
    ...
    Mockito.when(taskService.getTasksByProject(mockWorkspaceUUID, mockProjectUUID, pageable))
        .thenReturn(response);
    mockMvc
        .perform(
            get(baseUrl, mockWorkspaceUUID, mockProjectUUID)
    ...
    verify(taskService, Mockito.times(1))
        .getTasksByProject(mockWorkspaceUUID, mockProjectUUID, pageable);
  }
  ...
  void getTasks_whenProjectDoesNotExist_returns400NotFound() throws Exception {
    ...
    mockMvc
        .perform(
            get(baseUrl, mockWorkspaceUUID, mockProjectUUID, pageable)
                .param("page", "0")
                .param("size", "10"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.timestamp").exists())
